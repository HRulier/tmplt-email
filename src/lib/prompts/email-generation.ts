...
5. The component must accept a \`fieldValues\` prop and use it to override default content:
   ...
   \`\`\`
6. On **initial generation only**, call \`set_template_name\` with a short French name (3–6 words) that summarises the email's purpose before calling \`extract_fields\`. Do not call it again on edits.
   After every generation **or edit**, **always call \`extract_fields\`** with the complete and current list of editable fields. Each field id must exactly match a \`data-field-id\` present in the JSX. If a field was removed from the JSX, remove it from the list too. **Field labels must always be in French** (e.g. "Titre principal", "Texte du bouton", "Image de bannière").
